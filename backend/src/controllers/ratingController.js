...
export const addRating = async (req, res) => {
  try {
    const { technicianId, rating, review, jobId } = req.body
    ...
    if (!jobId) {
      return res.status(400).json({ message: "Job ID is required for rating" })
    }
    ...
    // 2. Prevent duplicate rating for this specific job
    ...
    // 5. Update technician rating stats
    const techData = techSnap.data()
    const newTotalReviews = (techData.totalReviews || 0) + 1
    const newAverageRating =
      ((techData.averageRating || 0) * (techData.totalReviews || 0) + numericRating) /
      newTotalReviews
